...
class Magmi_ProductImportEngine extends Magmi_Engine
{
    ...
    public function getOptionsFromValues($attid, $store_id, $optvals)
    {
        ...
        $sql .= " JOIN $t1 as opt ON opt.option_id=optvals.option_id AND opt.attribute_id=$attid";
        $sql .= " WHERE optvals.store_id=$store_id AND BINARY optvals.value IN  ($ovstr)";
    ...
    public function createOptionValue($optid, $store_id, $optval)
    {
		...
		$exists= $this->selectAll("SELECT * FROM $t WHERE option_id=$optid AND store_id=$store_id");
		
		if(count($exists) > 0){
			$optval_id = $this->insert("UPDATE $t SET value=\"$optval\" WHERE value_id=".$exists[0]['value_id']);
		}
		else{
    ...
    public function getOptionIds($attid, $storeid, $values)
    {
        ...
        foreach ($values as $val)
        {
            ...
            if (preg_match("|^(.*)::\[(.*)\]$|", $val, $matches))
            {
                ...
                $avalues[] = $matches[2]; //admin values
                // add admin value in admin value array
                $svalues[] = $matches[1]; //store values
            }
            else
            {
		...
		/**THIS IS DONE ALWAYS FOR ADMIN VALUES*********************************/
		// get Existing options for admin values & current attribute (store = 0)
            ...
            foreach ($new as $nval)
            {
                ...
                $this->createOptionValue($row["opvs"], 0, $nval);
        ...
        // if we are not on admin store view
        if ($storeid != 0){
			$exvals = array();
			...
			$existing_in_store = $this->getOptionsFromValues($attid, $storeid, $svalues);
			
			foreach ($existing_in_store as $optdesc)
			{
				$exvals[] = $optdesc["value"];
			}
			
			$new = array_merge(array_diff($svalues, $exvals));
			
			foreach($new as $new_value){
				$same_array_key=array_search($new_value,$svalues);
				$admin_array_val=$avalues[$same_array_key];
				
				//if exists in admin
				foreach($brows as $v){
					if($v['value']==$admin_array_val)
						$opvs=$v['opvs'];
				}
				
				if($opvs!=null){
					$this->createOptionValue($opvs, $storeid, $new_value);
