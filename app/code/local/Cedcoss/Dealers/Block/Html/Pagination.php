<?php
class Cedcoss_Dealers_Block_Html_Pagination extends Mage_Core_Block_Template
{
    ...
    { 
        ...
        $this->setTemplate('dealers/html/pagination.phtml');
    }
    ...
    { 
        ...
        if ($this->getRequest()->getParam($this->getPageVarName()))
		{
			$name = 'page_'.$id;
			
			$page_id = Mage::getSingleton('core/session')->getData($name);
			$page = $this->getRequest()->getParam($this->getPageVarName());
			if($id_url == $id)
			{ 
				Mage::getSingleton('core/session')->setData($name,$page);
				return $page;
			}
    ...
    public function getLimit($id)
    {
		...
		$limit = $_GET[$this->getLimitVarName()];
		if ($limit) 
		{ 
		...
		{ 
			$name = 'limit_'.$id;
			$limit_id = Mage::getSingleton('core/session')->getData($name);
			if($id_url == $id)
			{
				if (isset($limits[$limit]))
				{ 
					Mage::getSingleton('core/session')->setData($name,$limit);
					return $limit;
				}
			}
			else if(isset($limit_id))
				return $limit_id;
			else
				return $limits[10];
    ...
    public function setCollection($collection, $id)
    { 
		$this->_categoryId = $id;
		$this->_collection = $collection
            ->setCurPage($this->getCurrentPage($id));
        // If not int - then not limit
        if ((int) $this->getLimit($id)) {
            $this->_collection->setPageSize($this->getLimit($id));
        }
    ...
    public function isLimitCurrent($limit,$id)
    { //echo $this->getLimit($id); 
        return $limit == $this->getLimit($id);
    }

    public function isPageCurrent($page,$id)
    {
        return $page == $this->getCurrentPage($id);
    }
    ...
    {
        return $this->getPagerUrl(array($this->getPageVarName()=>$page,'id'=>$this->_categoryId));
    }

    public function getLimitUrl($limit)
    {
        return $this->getPagerUrl(array($this->getLimitVarName()=>$limit,'limit_id'=>$this->_categoryId));
    }
