...
{	
	...
	$giCity = geoip_open("media/GeoIP/GeoLiteCity.dat",GEOIP_STANDARD);
         ...
         if(!is_object($record))
         {
             Mage::log("geoip_record_by_addr was stumped by ".$giCity." and ".$ip."; it returned:".print_r($record,true),null,"supernova.log");
         }
         $infoData = array();
          $countryIds = array();
                    ...
                    foreach ($collection as $region) {
                        ...
                        if(is_object($region)) {
                           if($region->getCode()==$record->region){
                               ...
                               {
                                   $infoData['region_id']=   $region->getCode();
                               }
                               Mage::log("mess: ".$region->getCode()." vs ".$infoData['region_id'],null,"supernova.log");
                              }
                            
                        }
                        else
                        {
                            Mage::log("tried to get region ID for ".$record->region." off of:".print_r($region,true),null, "supernova.log");
                        }
   ...
   public function caculatorAction(){
       if(!isset($pageURL))
       {
		...
		if(array_key_exists('store',$_REQUEST)&&($_REQUEST['store']=='vc_retail')){
			$store = 'vc_retail';
		}
     	if($store == 'vc_retail' || $store == 'shopshowers' || $_REQUEST['store']=='vc_retail'){
		$pageURL = explode('?',$pageURL);
		$data = $pageURL[1]; //ugh...this needs further investigation (FLJ, 2/14/15)
            if(isset($_SERVER['HTTP_REFERER']))
            {
                Mage::log($_SERVER["SERVER_NAME"].":".$_SERVER['HTTP_REFERER'], null,"supernova.log");
            }
            else
            {
                Mage::log($_SERVER["SERVER_NAME"]." uhoh:".print_r($_SERVER,true), null,"supernova.log");
            }
            Mage::log("break 'product='on commas:".print_r($data,true),null,"supernova.log");
		}else{
		$data = $this->getRequest()->getPost();
		}
	  if($data){
              ...
              if($this->CheckLogin()){
                 ...
                 if(count($items)){
                     foreach($items as $item) {
					   ...
					   if(Mage::getModel('cataloginventory/stock_item')->loadByProduct($_products)->getQty()<$req['qty']  || !$_products->isSaleable())
						 {	
							return;
						 }
			   ...
			   if($store == 'vc_retail' || $store == 'shopshowers'){
				...
				'options' => array(
					21 => $value[count($value)-1],
					19 => $value[count($value)-2],
				),
				'qty' => 1,
				);
			   ...
			   for($i=0;$i<count($data);$i++){
				   ...
				   //$quote->addProduct($product,$qt);
					...
					if($value[6] == 0){ $option_val[6] = 0; }
					if($value[7] == 0){ $option_val[7] = 0; }
					if($value[8] == 0){ $option_val[8] = 0; }
					if($value[9] == 0){ $option_val[9] = 0; }
				   ...
				   $product = Mage::getModel('catalog/product')->load($temp[0]);
					...
					'bundle_option' => array(
						...
						$option_id[6] => $option_val[6],
						$option_id[7] => $option_val[7],
						$option_id[8] => $option_val[8],
						$option_id[9] => $option_val[9]
					),
					'bundle_option_qty' => array(
						$option_id[0] => $value[0],
						$option_id[1] => $value[1],
				   ...
				   foreach($data as $productId => $qty){
						   $request['qty'] = $qty;
							...
							$product = $model->load($productId);
							 if(Mage::getModel('cataloginventory/stock_item')->loadByProduct($product)->getQty()<$qty || !$product->isSaleable())
							 {	
								return;
							 }
							$quote->addProduct($product,$qt);
